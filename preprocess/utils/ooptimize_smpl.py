# Coded on https://github.com/apple/ml-neuman/blob/0149d258b2afe6ef65c91557bba9f874675871e4/preprocess/optimize_smpl.py

'''
Optimize SMPL parameters with keypoints from Sapiens.
...
def coco133_to_smpl(coco2d, selected_keypoints=None):
    '''
    ...
    smpl2d = torch.zeros((45, 2))
    
    # Only optimize ankle and feet
    ...
    # left foot
    smpl2d[29] = coco2d[17] # 18 19 20 -> leftFeet
    smpl2d[30] = coco2d[18]
    smpl2d[31] = coco2d[19]
    
    # right foot
    smpl2d[32] = coco2d[20] # 21 22 23 -> rightFeet
    smpl2d[33] = coco2d[21]
    smpl2d[34] = coco2d[22]
...
def turn_smpl_gradient_on():
    '''
    only apply gradients on feet joints.
    '''
    grad_mask = np.zeros([45, 3])   ### CHECK
    visible = ['Elbow', 'wrist']
...
def optimize_smpl(camera, data_item, body_model, scale=1.0, num_iters=100):
    ...
    betas = torch.nn.Parameter(smpl_betas.float().to('cuda'), requires_grad=False)
    ...
    joints_target = coco133_to_smpl(joints_target[:, :2]).float().to(device)
    ...
    for i in range(num_iters):
        ...
        # cv2.imwrite('./viz_debug/test_silhouette.png', silhouette_np)
        ...
        # cv2.imwrite('./viz_debug/test_silhouette_annot.png', silhouette_np)
...
def main(kit_dir, num_iters = 100):
    device = torch.device('cuda')
    
    body_model = SMPL(
        os.path.join(os.path.dirname(os.path.abspath(__file__)), '../../data/human_models/smpl'),
        device=device
    )
    ...
    for i in tqdm(range(data_length), total=data_length, desc='SMPL Optimization'):
        ...
        smpl_poses.append(new_smpl)  
        smpl_betas.append(data_item['betas'])
    
    smpl_betas = torch.stack(smpl_betas, dim=0)
    smpl_poses = torch.stack(smpl_poses, dim=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_iters_per_frame', type=int, default=100, required=False)
