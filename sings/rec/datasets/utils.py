...
from sings.rec.utils.graphics import fov2focal, get_projection_matrix
from sings.rec.utils.geometry.rotations import batch_look_at_th
...
def get_predefined_pose(pose_type, device='cuda'):
    ...
    elif pose_type == 'little_little_a_pose':
        body_pose = torch.zeros((1, 69), dtype=torch.float32, device=device)
        body_pose[:, 2] = 0.02
        body_pose[:, 5] = -0.02
        body_pose[:, 47] = -0.9
