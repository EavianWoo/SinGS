...
from smplx.body_models import VertexJointSelector
...
class SMPLH(SMPL):
    ...
    def __init__(self,
        ...
        super(SMPLH, self).__init__(model_path,
        ...
        # if osp.isdir(model_path):
        #     model_fn = 'SMPLH_{}.{ext}'.format(gender.upper(), ext='pkl')
    ...
    def verts_transformations(self,
        ...
        left_hand_pose = left_hand_pose.to('cuda')
        right_hand_pose = right_hand_pose.to('cuda')
        full_pose = torch.cat([poses[:, :66],
                               left_hand_pose,
                               right_hand_pose], dim=1)
        
        full_pose += self.pose_mean.to('cuda')
        
        
        L, vertices = lbs(betas, full_pose, self.v_template,
    ...
    def forward(self,
                ...
                return_joints=False,
                return_tensor=True):
        ''' Forward pass for SMPLH model

        Parameters
        ----------
        betas: torch.tensor, optional, shape Bx16
                If given, ignore the member variable `betas` and use it
                instead. For example, it can used if shape parameters
                `betas` are predicted from some external model.
                (default=None)
        ...
        # extend joints
        joints = self.vertex_joint_selector(vertices.cpu(), joints.cpu())
