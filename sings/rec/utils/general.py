...
def create_video(img_folder, output_fname, fps=20, ext='png'):
    os.makedirs(os.path.dirname(output_fname), exist_ok=True)
    cmd = f"ffmpeg -hide_banner -loglevel error -framerate {fps} -pattern_type glob -i '{img_folder}/*.{ext}' \
...
def get_cosine_annealing_lr(
    lr_init, lr_final, lr_delay_steps, T_max
):
    def helper(step):
        if step < lr_delay_steps:
            return lr_init
        
        step_after_delay = step - lr_delay_steps
        T_max_after_delay = T_max - lr_delay_steps
        
        cosine_term = math.cos(math.pi * step_after_delay / T_max_after_delay)
        lr = lr_final + 0.5 * (lr_init - lr_final) * (1 + cosine_term)
        return lr
