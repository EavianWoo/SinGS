...
from sings.rec.utils.general import inverse_sigmoid
...
@torch.no_grad()
def save_ply(human_gs_out, path, pose='canonical'):
...
## obeserve levels
@torch.no_grad()
def save_ply_by_level(human_gs_out, save_path):
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    
    xyz = human_gs_out['xyz_canon'].cpu().numpy()
    level = human_gs_out['level_id'].cpu().numpy()
    colors = np.array(list(map(map_level_to_color, level)))
    
    dtype_full = [(attr, 'f4') for attr in ['x', 'y', 'z']]
    dtype_full.extend([(attr, 'u1') for attr in ['red', 'green', 'blue']])

    elements = np.empty(xyz.shape[0], dtype=dtype_full)
    attributes = np.concatenate((xyz, colors), axis=1)
    elements[:] = list(map(tuple, attributes))
    el = PlyElement.describe(elements, 'vertex')
...
def get_vis_mesh(mean, scale, rotation, colors, transl=None, normals=None):
    ...
    for i in range(sp_verts.shape[0]):
        ...
        # vertex_colors = np.tile(c[:3], (sp_verts[i].shape[0], 1))  # Tile the RGB color for all vertices
...
def generate_colormap(b1, g1, r1, b2, g2, r2, n=15):
    r_values = np.linspace(r1, r2, n)
    g_values = np.linspace(g1, g2, n)
    b_values = np.linspace(b1, b2, n)
    
    colormap = np.column_stack((r_values, g_values, b_values)).astype(int)
    return colormap


from matplotlib import cm
def save_mesh(verts, faces, vertex_label, smpl_mesh=None):
    colormap_1 = generate_colormap(217, 112, 14, 91, 115, 20, n=8)
    colormap_2 = generate_colormap(91, 115, 20, 137, 166, 187, n=7)
    colormap = np.concatenate([colormap_1, colormap_2], axis=0)
    vertex_label = vertex_label.squeeze()
     
    # colormap = cm.get_cmap('jet', 15)
    # label_colors = colormap(np.linspace(0, 1, 15))
    # vertex_colors = np.array([label_colors[label] for label in vertex_label])
    vertex_colors = np.array([colormap[label] for label in vertex_label])

    # trimesh color format: RGBA (0-255)
    vertex_colors = (vertex_colors[:, :4] * 255).astype(np.uint8)
    
    color_mesh = trimesh.Trimesh(vertices=verts, faces=faces, vertex_colors=vertex_colors)
    # color_mesh_2 = trimesh.Trimesh(vertices=smpl_mesh.vertices, faces=smpl_mesh.faces, vertex_colors=vertex_colors)
    
    color_mesh.export('color_mesh_A_pose.ply')
    

def viz_region_label(verts, faces, v_label): # 在不增加球时可用

    # get colormap
    num_labels = len(set(v_label))
    colormap = cm.get_cmap('jet', num_labels)
    label_colors = colormap(np.linspace(0, 1, num_labels))
    
    # allocate color by label
    vertex_colors = np.array([label_colors[label] for label in v_label])

    # trimesh format -  RGBA (0-255)
    vertex_colors = (vertex_colors[:, :4] * 255).astype(np.uint8)
    ...
    rect_size_y = 0.1  # Vertical size of the rectangles
    rect_size_x = mesh_width / num_labels
    legend_meshes = []
    # Offset for placing the colorbar below the mesh
    offset_y = verts[:, 1].min() - 0.5  # Adjust Y position below the mesh

    # Create the colorbar as a series of rectangles
    for i, color in enumerate(label_colors):
        # Define rectangle vertices
        rect_verts = np.array([
            [min_x + i * rect_size_x, offset_y, 0],  # Bottom left
            [min_x + (i + 1) * rect_size_x, offset_y, 0],  # Bottom right
            [min_x + (i + 1) * rect_size_x, offset_y + rect_size_y, 0],  # Top right
            [min_x + i * rect_size_x, offset_y + rect_size_y, 0]  # Top left
        ])
        rect_faces = np.array([[0, 1, 2], [0, 2, 3]])  # Two triangles form a rectangle
        rect_colors = np.tile((np.array(color[:4]) * 255).astype(np.uint8), (4, 1))  # Color for each vertex
        
        # created colored rectangle
        legend_mesh = trimesh.Trimesh(vertices=rect_verts, faces=rect_faces, vertex_colors=rect_colors)
        legend_meshes.append(legend_mesh)
        
    # add to scene
    scene = mesh.scene()
    for legend_mesh in legend_meshes:
        scene.add_geometry(legend_mesh)
